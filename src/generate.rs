...
use crate::openai::{GenerationMetrics, StreamOutput, generate_message_with_stream_output};
...
pub fn run_generate() -> Result<(), String> {
    ...
    let cfg = load_config_for_interactive_use()?;
    ...
    let (message, metrics) = generate_message_with_stream_output(&cfg, &repo_ctx, stream_output)?;
